...
// +----------------------------------------------------------------------
namespace plugin\saicode\utils;
...
class Compile
{
    ...
    public function getIfValue($strTemplate, $element)
    {
        ...
        if (isset($element['column_name'])) {
            ...
            $element['span'] = $element['span'] ?? $this->value['span'];
            ...
            if (in_array($element['view_type'], ['select', 'cascader', 'treeSelect'])) {
                $element['url'] = $element['options']['url'] ?? '';
                $element['value'] = $element['options']['field_value'] ?? 'value';
                $element['label'] = $element['options']['field_label'] ?? 'label';
            }
            if (in_array($element['view_type'], ['inputNumber', 'slider'])) {
                $element['min'] = $element['options']['min'] ?? 1;
                $element['max'] = $element['options']['max'] ?? 100;
                $element['step'] = $element['options']['step'] ?? 1;
            }
